"""KfW source discoverer for the cronjob pipeline.
...
+ ``pipeline_core.submit_candidates(candidates, source="kfw")`` for
download / OCR / submit.

The KfW source uses two paths:

1. **Primary BS4 path** — ``extract_kfw_pdf_urls`` scans the listing
   at
   ``https://www.kfw-entwicklungsbank.de/Service/Procurement-Regulations/``
   for ``.pdf`` anchors on the ``kfw.de`` / ``kfw-entwicklungsbank.de``
   hosts whose href / path / text / title / aria-label carries one of
   the KfW signal tokens (``call for proposals``, ``request for
   proposals``, ``expression of interest``, ``procurement``). On
   ``kfw-entwicklungsbank.de`` only PDFs under the procurement
   subpaths (``/service/procurement-regulations/``,
   ``/document-center/``,
   ``/pdf/download-center/pdf-dokumente-richtlinien/``) are kept.
...
2. **Playwright fallback** — only triggered when the BS4 path yields
   ...
   feed back into ``extract_kfw_pdf_urls``. Playwright is imported
...
KFW_MAX_CANDIDATES_PER_RUN = int(
    os.environ.get("KFW_MAX_CANDIDATES_PER_RUN", "50"),
)
KFW_FETCH_MAX_ATTEMPTS = int(os.environ.get("KFW_FETCH_MAX_ATTEMPTS", "3"))
KFW_FETCH_BACKOFF_SECONDS = float(os.environ.get("KFW_FETCH_BACKOFF_SECONDS", "2"))
KFW_FETCH_TIMEOUT_SECONDS = int(os.environ.get("KFW_FETCH_TIMEOUT_SECONDS", "30"))


def extract_kfw_pdf_urls(listing_doc: BeautifulSoup | str, listing_url: str) -> list[str]:
    """Discover PDF URLs from the KfW listing HTML.

    Verbatim port of ``extract_kfw_pdf_urls`` from the FastAPI repo.
    The extractor keeps only ``.pdf`` anchors on the ``kfw.de`` /
    ``kfw-entwicklungsbank.de`` hosts that carry one of the KfW
    signal tokens (``call for proposals``, ``request for proposals``,
    ``expression of interest``, ``procurement``). On
    ``kfw-entwicklungsbank.de`` only PDFs under the procurement
    subpaths are kept. URL fragments are stripped before the URL is
    canonicalised.
    """
    ...
    for link in soup.find_all("a"):
        ...
        if (
            host != "kfw.de"
            and not host.endswith(".kfw.de")
            and host != "kfw-entwicklungsbank.de"
            and not host.endswith(".kfw-entwicklungsbank.de")
        ):
            continue

        if (
            (host == "kfw-entwicklungsbank.de" or host.endswith(".kfw-entwicklungsbank.de"))
            and "/service/procurement-regulations/" not in path_lower
            and "/document-center/" not in path_lower
            and "/pdf/download-center/pdf-dokumente-richtlinien/" not in path_lower
        ):
            continue
...
async def _collect_listing_anchors(listing_url: str) -> str:
    ...
    aria-label. The fragment is fed back into ``extract_kfw_pdf_urls``
...
def _run_playwright_fallback(listing_url: str, *, stats: dict[str, int]) -> list[str]:
    ...
    try:
        ...
        log_source_failure(
            "Playwright is not installed; skipping KfW fallback for %s",
            listing_url,
        ...
        log_source_failure(
            "Failed processing KfW listing page %s: %s",
            listing_url,
            exc,
            exc=exc,
        )
    ...
    return extract_kfw_pdf_urls(listing_html, listing_url)
...
def build_candidate(
    ...
    *,
    ...
    """Build a ``kind="pdf"`` candidate for the KfW source."""
    if origin is None:
...
def discover_candidates() -> tuple[dict[str, int], list[dict[str, Any]]]:
    """Discover KfW edital PDF candidates from the listing page.
    ...
    try:
        listing_html = fetch_html_with_retry(
            KFW_LISTING_URL,
            timeout=KFW_FETCH_TIMEOUT_SECONDS,
            max_attempts=KFW_FETCH_MAX_ATTEMPTS,
            backoff_seconds=KFW_FETCH_BACKOFF_SECONDS,
            allowed_status_codes=(401, 403, 404, 410),
        )
        stats["listings_fetched"] += 1
    except Exception as exc:
        log_source_failure(
            "Failed to fetch KfW listing %s: %s",
            KFW_LISTING_URL,
            exc,
            exc=exc,
        )
    ...
    if not pdf_urls:
        ...
        pdf_urls = _run_playwright_fallback(KFW_LISTING_URL, stats=stats)

    for pdf_url in pdf_urls:
        if pdf_url in seen_pdfs:
            continue
        seen_pdfs.add(pdf_url)
        candidate = build_candidate(pdf_url, listing_url=KFW_LISTING_URL)
        candidates.append(candidate)
        if len(candidates) >= KFW_MAX_CANDIDATES_PER_RUN:
            stats["candidate_cap_reached"] = 1
            print(
                f"Stopping after candidate cap {KFW_MAX_CANDIDATES_PER_RUN}",
                file=sys.stderr,
            )
...
def main() -> int:
    ...
    print(f"KfW discovery stats: {stats}")
    print(f"KfW candidates discovered: {len(candidates)}")
    ...
    print(f"KfW processing stats: {stats}")

    if candidates and stats["ocr_successes"] == 0:
        print(
            "error: all discovered KfW candidates failed download/OCR; nothing will be submitted",
            file=sys.stderr,
        )
        return 1

    result = pipeline_core.submit_candidates(processed, source="kfw")
    print(f"Render candidate submission: {result}")

    if candidates and result.get("submitted", 0) == 0:
        print(
            "error: discovered KfW candidates produced no Render submissions",
            file=sys.stderr,
        )
        return 1
