...
1. Direct ``.pdf`` anchors on the listing page itself (e.g. to
   ``wwfbrnew.awsassets.panda.org/downloads/...``) — these become
   candidates immediately.
2. Detail-page anchors that target
   ``/sobrenos/aquisicoesecontratacoes/?uNewsID=<digits>`` on the
   listing host — each detail page is then fetched via
...
def extract_wwf_detail_urls(listing_html: str, listing_url: str) -> list[str]:
    ...
    under ``/sobrenos/aquisicoesecontratacoes`` and whose query
    carries a numeric ``uNewsID``.
    """
    ...
    for link in soup.find_all("a"):
        ...
        news_ids = parse_qs(normalized.query).get("uNewsID")
        if not news_ids or any(not news_id.isdigit() for news_id in news_ids):
            continue
...
def discover_candidates(
    *,
    ...
    listing_pdfs: list[str] = []
    for link in listing_soup.find_all(
        "a", href=lambda href: bool(href and looks_like_pdf_url(href)),
    ):
        if not isinstance(link, Tag):
            continue
        href = link.get("href")
        if not isinstance(href, str) or not href:
            continue
        listing_pdfs.append(urljoin(WWF_LISTING_URL, href))

    detail_urls = extract_wwf_detail_urls(listing_html, WWF_LISTING_URL)
    detail_pdf_records: list[tuple[str, str]] = []
    details_fetched = 0
    for detail_url in detail_urls:
        if details_fetched >= WWF_MAX_DETAILS_PER_RUN:
            ...
            break
        try:
            page_pdfs = discover_pdf_urls_on_page(detail_url, stats=stats)
        except Exception as exc:
            log_source_failure(
                "Failed to discover PDFs on WWF detail %s: %s",
                detail_url,
                exc,
                exc=exc,
            )
        ...
        for pdf_url in page_pdfs:
            detail_pdf_records.append((pdf_url, detail_url))

    for pdf_url in listing_pdfs:
        if pdf_url in seen_pdfs:
            continue
        seen_pdfs.add(pdf_url)
        candidate = build_candidate(
            pdf_url,
            listing_url=WWF_LISTING_URL,
    ...
    for pdf_url, detail_url in detail_pdf_records:
        if pdf_url in seen_pdfs:
            continue
        seen_pdfs.add(pdf_url)
        candidate = build_candidate(
