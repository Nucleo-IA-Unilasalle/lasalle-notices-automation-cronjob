...
- `scripts/check_source_catalog_parity.py` with `config/source_catalog_contract.json` gates pinned A-manifest parity offline and live comparison on main push/daily/manual (pins older than 14 days rejected; 39th workflow `source-catalog-parity.yml`). Any future paused registry entry is an explicit local execution hold reported separately, not healthy coverage.
...
- Source-local discovery fixes must retain official lifecycle signals and stable record/document identities. The consolidated 2026-09-14 technical audit under `docs/evidence/technical-audit-2026-09-14-FIX-REVIEW.md` is the regression basis for the 2026-09-21 operator-authorized all-source production expansion; runtime health still depends on observed scheduled executions.
- Staging/evidence lives in `docs/STAGING-RUNBOOK.md` (in-progress checklist) and `docs/evidence/` (dated observed results, index, snapshot validation, independent-audit template, per-source folders and 48 h template). Recorded deployment/contract probes do not replace production telemetry. Hosted staging uses the existing staging API/frontend and an isolated free database; never substitute production credentials or data to unblock it.
- `scripts/run_independent_source_audit.py` is a repository-owned, selected-page diagnostic only: it is not independent evidence, never signs off RR-05, fails closed on any official-page/discovery error or cap, and returns nonzero even when its URL comparison has no blockers. Selected-link coverage is checked separately from lifecycle-aware fidelity so unknown status cannot hide a missing selected link; unknown lifecycle status remains unknown. Independent audits require separately prepared official-source inventory, retained provenance, and reviewer sign-off.
- `scripts/run_staging_failure_injection.py` is fixed to the staging API and may mutate only staging lease state. A live run requires both `--allow-live-staging-mutation` and `STAGING_FAILURE_INJECTION_ACK=staging-only`; production is forbidden. Track every admitted HTTP 201 lease before returning, perform best-effort `outcome=noop` cleanup in `finally`, return nonzero for any failed gate or cleanup, and never print or persist raw secrets or claim tokens. Offline deterministic tests cover the guard, cleanup under exception, and nonzero outcomes.
