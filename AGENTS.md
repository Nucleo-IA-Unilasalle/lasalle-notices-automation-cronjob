...
- Closed-beta execution is default-deny and source-generic. `CLOSED_BETA_SOURCE_ALLOWLIST` must be exactly one known ingest-mode candidate source, must match Repo A's `SOURCE_ADMISSION_SOURCE`, and its registry `submission_contract` must match Repo A's `SOURCE_ADMISSION_CONTRACT` (`candidate` for the first wave). Group matrices emit only that source, non-owning groups skip discovery, PNCP runs only when explicitly selected, and every managed worker rechecks admission before constructing the claim client. Repo A is the authoritative fence; this worker-side filter prevents unselected jobs from consuming full runner allocations but cannot fence old refs or direct API calls by itself.
...
- Source-local discovery fixes must retain official lifecycle signals, stable record/document identities, and the configured paused/audit rollout holds. The consolidated 2026-09-14 technical audit under `docs/evidence/technical-audit-2026-09-14-FIX-REVIEW.md` is regression evidence only; it does not close independent-audit, canary, soak, or release gates.
