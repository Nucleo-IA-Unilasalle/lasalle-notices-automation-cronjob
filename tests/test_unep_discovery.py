"""Unit tests for ``scripts/discover_unep_candidates.py``.

Ports the UNEP source from
``lasalle-notices-automation/app/services/scraper/sources/unep.py`` into
the cronjob, locking the discovery contract before Phase 3 ships.
Tests exercise the ``extract_unep_pdf_urls`` helper (signal filter,
host allowlist, tracking-query sanitisation, fragment stripping),
single-page listing discovery, year guard, ``is_likely_edital``
...
FIXTURES_DIR = Path(__file__).resolve().parent / "fixtures" / "sources" / "unep"


LISTING_FIXTURE = FIXTURES_DIR / "listing.html"


LISTING_URL = "https://www.unep.org/global-framework-chemicals/gfc-fund/applying-funding"
...
class TestExtractUnepPdfUrls:
    ...
    def test_listing_yields_signal_pdf_urls(self) -> None:
        ...
        discovered = extract_unep_pdf_urls(
...
# The fixture has:
    ...
    def test_rejects_external_hosts(self) -> None:
        ...
        <html><body>
          ...
          <a href="https://www.unep.org/call-for-proposals.pdf">Valido</a>
        </body></html>
        """
        assert extract_unep_pdf_urls(html, LISTING_URL) == [
            "https://www.unep.org/call-for-proposals.pdf",
        ]
    ...
    def test_rejects_non_pdf_anchors(self) -> None:
        ...
        <html><body>
          ...
          <a href="/resources/page/call-for-proposals">Sem extensao</a>
          <a href="/resources/call-for-proposals.pdf">PDF</a>
        </body></html>
        """
        assert extract_unep_pdf_urls(html, LISTING_URL) == [
            "https://www.unep.org/resources/call-for-proposals.pdf",
        ]
    ...
    def test_strips_tracking_query_params(self) -> None:
        ...
        html = """
        <html><body>
          <a href="/resources/call-for-proposals.pdf?utm_source=fb&fbclid=abc&page=2">Tracking</a>
        </body></html>
        """
        assert extract_unep_pdf_urls(html, LISTING_URL) == [
            "https://www.unep.org/resources/call-for-proposals.pdf?page=2",
        ]
    ...
    def test_strips_url_fragment(self) -> None:
        ...
        html = """
        <html><body>
          <a href="/resources/call-for-proposals.pdf#page=2">Fragment</a>
        </body></html>
        """
        assert extract_unep_pdf_urls(html, LISTING_URL) == [
            "https://www.unep.org/resources/call-for-proposals.pdf",
    ...
    def test_canonicalises_bare_unep_org_to_www(self) -> None:
        from discover_unep_candidates import extract_unep_pdf_urls

        html = """
        <html><body>
          <a href="https://unep.org/resources/call-for-proposals.pdf">Bare host</a>
        </body></html>
        """
        assert extract_unep_pdf_urls(html, LISTING_URL) == [
            "https://www.unep.org/resources/call-for-proposals.pdf",
...
class TestListingUrl:
    def test_default_listing_url(self) -> None:
        from discover_unep_candidates import UNEP_LISTING_URL
...
class TestYearGuard:
    def test_default_min_notice_year_is_2026(self, monkeypatch: pytest.MonkeyPatch) -> None:
        monkeypatch.delenv("UNEP_MIN_NOTICE_YEAR", raising=False)
        module = importlib.reload(
            __import__("discover_unep_candidates"),
        )
        assert module.UNEP_MIN_NOTICE_YEAR == 2026

    def test_min_notice_year_override(self, monkeypatch: pytest.MonkeyPatch) -> None:
        monkeypatch.setenv("UNEP_MIN_NOTICE_YEAR", "2027")
        module = importlib.reload(
            __import__("discover_unep_candidates"),
        )
        assert module.UNEP_MIN_NOTICE_YEAR == 2027
        monkeypatch.delenv("UNEP_MIN_NOTICE_YEAR", raising=False)
        importlib.reload(__import__("discover_unep_candidates"))

    def test_year_extracted_from_pdf_path(self) -> None:
        from discover_unep_candidates import _extract_year_from_url

        assert _extract_year_from_url(
            "https://www.unep.org/resources/report/call-for-proposals-2026.pdf",
        ) == 2026
    ...
    def test_year_below_minimum_is_rejected(self) -> None:
        from discover_unep_candidates import _passes_year_guard

        url_2024 = "https://www.unep.org/resources/call-for-proposals-2024.pdf"
        url_2026 = "https://www.unep.org/resources/call-for-proposals-2026.pdf"
        url_unknown = "https://www.unep.org/resources/call-for-proposals.pdf"
...
class TestEditalPrefilter:
    def test_candidate_passes_is_likely_edital_with_edital_in_url(self) -> None:
        from discover_unep_candidates import _candidate_passes_edital_prefilter

        url = "https://www.unep.org/resources/edital-2026.pdf"
    ...
    def test_candidate_rejected_when_filename_matches_exclusion(self) -> None:
        ...
        url = "https://www.unep.org/resources/resultado-final-2026.pdf"
...
class TestBuildCandidate:
    def test_build_candidate_records_origin_and_year(self) -> None:
        ...
        from discover_unep_candidates import build_candidate

        url = "https://www.unep.org/resources/call-for-proposals-2026.pdf"
    ...
    def test_build_candidate_returns_none_for_pre_min_year(self) -> None:
        ...
        url = "https://www.unep.org/resources/call-for-proposals-2025.pdf"
...
class TestDiscoverCandidates:
    def test_listing_yields_signal_pdf_candidates(self) -> None:
    ...
    def test_year_filter_excludes_pre_2026_signal_pdf(self) -> None:
        ...
        html = """
        <html><body>
          <a href="/resources/call-for-proposals-2025.pdf">2025</a>
          <a href="/resources/call-for-proposals-2026.pdf">2026</a>
        </body></html>
        """
        with patch_request_with_safe_redirects(
        ...
        urls = [c["url"] for c in candidates]
        assert "https://www.unep.org/resources/call-for-proposals-2025.pdf" not in urls
        assert "https://www.unep.org/resources/call-for-proposals-2026.pdf" in urls
...
class TestSubmitHandoff:
    def test_main_calls_submit_candidates_with_unep_source(self) -> None:
        import discover_unep_candidates as dpc

        candidate = {
            "url": "https://www.unep.org/example.pdf",
            "kind": "pdf",
            "metadata": {"source": "unep"},
            "worker_result": {
                "ocr_markdown": "# Edital",
                "content_hash": "h",
                "content_length": 100,
        ...
        assert call_args.kwargs["source"] == "unep"
